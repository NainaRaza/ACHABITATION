...
public class PersonService {
    ...
    @Transactional
    public PersonResponse createForCurrentUser(UUID tripId, CurrentUserPersonCreateRequest request, UserEntity user) {
        authorizationService.requireWritable(tripId, user);
        TripEntity trip = tripService.getRequired(tripId);

        personRepository.findByTripIdAndLinkedUserId(tripId, user.getId())
                .ifPresent(existing -> { throw new IllegalArgumentException("Ton compte est déjà lié à une personne dans ce voyage."); });
        ...
        String normalizedName = mapper.normalizeName(displayName);
        if (personRepository.existsByTripIdAndNormalizedName(tripId, normalizedName)) {
            throw new IllegalArgumentException("Une personne porte déjà ce nom dans ce voyage. Choisis le guest correspondant ou un autre nom.");
        }
        ...
        replacePresencePeriods(person, request == null ? null : request.presencePeriods());
        validatePerson(person);
        person = personRepository.save(person);
        auditService.log(trip, user, AuditAction.PERSON_CREATED, "PERSON", person.getId(), "Personne liée au compte créée : " + person.getName());
    ...
    private void applyDefaultLinkedPersonFields(PersonEntity person) {
        person.setWeightMode(WeightMode.AVERAGE);
        person.setAdvancedLivingRest(false);
        ...
        person.setVegetarian(false);
        person.setNoAlcohol(false);
