...
echo "[4/11] Création participant·es"
...
echo "[9/11] Audit"
api GET "$BASE_URL/trips/$TRIP_ID/audit-logs" "$OWNER_TOKEN" >/dev/null

echo "[10/11] Logout serveur"
api POST "$BASE_URL/auth/logout" "$OWNER_TOKEN" >/dev/null
STATUS_AFTER_LOGOUT=$(curl -sS -o /dev/null -w "%{http_code}" "$BASE_URL/auth/profile" -H "Authorization: Bearer $OWNER_TOKEN")
if [[ "$STATUS_AFTER_LOGOUT" != "401" ]]; then
  echo "Logout invalide: /auth/profile retourne $STATUS_AFTER_LOGOUT au lieu de 401" >&2
  exit 1
fi

echo "[11/11] Smoke test OK — voyage $TRIP_ID"
