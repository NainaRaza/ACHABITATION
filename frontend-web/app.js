...
function bindEvents() {
    ...
    $("dismissPostJoinClaimBtn")?.addEventListener("click", () => hidePanel("postJoinClaimPanel"));
    $("postJoinCreateSelfBtn")?.addEventListener("click", () => createCurrentUserPerson());
    ...
    $("addCurrentUserPersonBtn")?.addEventListener("click", () => createCurrentUserPerson());
...
function renderPostJoinClaimPanel() {
    const panel = $("postJoinClaimPanel");
    const container = $("postJoinGuestOptions");
    if (!panel || !container) return;
    if (!state.user?.devToken || !state.selectedTrip?.id || state.pendingClaimTripId !== state.selectedTrip.id || currentUserAlreadyLinkedInSelectedTrip()) {
    ...
    if (!guests.length) {
        ...
        container.querySelectorAll("[data-post-join-link]").forEach(button => {
            button.addEventListener("click", () => linkPersonToCurrentUser(button.dataset.postJoinLink));
        });
    }
    panel.classList.remove("hidden");
}

async function createCurrentUserPerson() {
    if (!ensureTripSelected()) return;
    if (!state.user?.devToken) {
